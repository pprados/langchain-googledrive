...
_IMPORT_RE = re.compile(
    r"from\s+(langchain_googledrive\.\w+(\.\w+)*?)\s+import\s+"
...
def find_files(path: str) -> Generator:
...
def get_full_module_name(module_path: str, class_name: str) -> str:
    """Get full module name using inspect"""
    module = importlib.import_module(module_path)
    class_ = getattr(module, class_name)
    return cast(ModuleType, inspect.getmodule(class_)).__name__
...
def main() -> None:
    """Main function"""
    args = get_args()
    global_imports: Dict[str, Dict[str, str]] = {}
...
def replace_imports(file: str) -> List[Dict[str, str]]:
